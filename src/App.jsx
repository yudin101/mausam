...
import { faTemperatureHalf, faWind, faDroplet, faGauge, faEye, faSun, faSearch } from "@fortawesome/free-solid-svg-icons";
...
function App() {
	...
	// This is for the InfoBox component just below the MainWeather component
	const infoBoxItems = [
		{
	...
	function getDayForecast(data) {
	  ...
	  const filteredData = data.filter(item => (new Date(item.dt * 1000).getDate()) !== new Date().getDate());
		...
		<>
			<div className="leftBox">
				<div className="inputBox">
					<input id="citySearchInput" onChange={(e) => setCity(e.target.value)} type="text" placeholder="Search for cities" />
					<button onClick={citySearch}><FontAwesomeIcon icon={faSearch} /></button>
				</div>
				...
				<div className="mainWeatherBox">
					...
					<img className="weatherImg" src={imgURL} alt="Current Weather Picture"/>
				</div>

				<div className="infoBoxContainer">
					{infoBoxItems.map((item, index) => <InfoBox key={index} icon={item.icon} text={item.text} value={item.value} />)}
				</div>
			</div>
				...
				<div className="timeForecast">
					...
					<div className="timeForecastBox">
						{forecastData.list.slice(0, 3).map((item, index) => (
							<TimeForecast key={index} time={dtToTime(item.dt)} img={item.weather[0].icon} temp={tempConv(item.main.temp)} />
						))}
					</div>
				</div>
				...
				<div className="dayForecast">
					...
					<ul>
						{getDayForecast(forecastData.list).map((item, index) => (
							<DayForecast key={index} day={dtToDay(item.dt)} img={item.weather[0].icon} desc={item.weather[0].main} maxTemp={tempConv(item.main.temp_max)} minTemp={tempConv(item.main.temp_min)} />
						))}
					</ul>
				</div>
			</div>
